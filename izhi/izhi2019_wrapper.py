...
type2019 = collections.OrderedDict([
...
class IzhiCell(object):
    """
    Create an izhikevich cell based on 2019 parameterization using izhi2019.mod
    ...
    def __init__(self, celltype='RS', cellid=None):
        ...
        if cellid is None:
            ...
            self.cellid = cellid
        if celltype not in type2019.keys():
            raise KeyError('izhi2019 cell type not recognized: %s' % str(celltype))
        self.sec = h.Section(name='izhi2019' + celltype + str(self.cellid))
        self.sec.L, self.sec.diam, self.sec.cm = 10, 10, 31.831  # empirically tuned
        self.izh = h.Izhi2019(0.5, sec=self.sec)
    ...
    def reparam(self, celltype='RS', cellid=None):
        ...
        self.izh.d, self.izh.celltype = type2019[celltype]
